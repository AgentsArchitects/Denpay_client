...
import xeroService, { XeroContactGroupData, XeroTenant } from '../../services/xeroService';
import './XeroList.css';

const { Option } = Select;

const XeroContactGroups: React.FC = () => {
  const shortId = (id: string) => id.replace(/-/g, '').substring(0, 8).toUpperCase();
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<XeroContactGroupData[]>([]);
  ...
  const fetchContactGroups = async () => {
    setLoading(true);
    try {
      const response = await xeroService.getContactGroups({
      ...
      console.log('Failed to fetch contact groups:', error);
      setData([]);
      setTotal(0);
    } finally {
      setLoading(false);
  ...
  const columns: ColumnsType<XeroContactGroupData> = [
    {
      title: 'Name',
      dataIndex: 'name',
      key: 'name',
    ...
    <div className="xero-list-container">
      ...
      <Card className="xero-list-card">
        <div className="table-toolbar">
          ...
          />
          <span style={{ color: '#6B7280' }}>Total: {total} groups</span>
        </div>
          ...
          locale={{
            emptyText: (
              <div className="empty-state">
                ...
                <p className="empty-text">No contact groups found. Please sync data from Xero first.</p>
              </div>
            ),
          }}
        />
...
export default XeroContactGroups;
